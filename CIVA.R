...
CIVA <- function(X, nc, R, starts, parallel = FALSE, verbose = TRUE, initP = NULL){
  ...
  for(i in 1:starts){
    ...
    repeat({
      ...
      newclus <- SearchEmptyClusters(nClus = R,
                                     newcluster = UpdatePInfo$newclus,
                                     SSminVec = UpdatePInfo$SSminVec)
      
      table(UpdatePInfo$newclus)
      ...
      if( (criterion[1] - criterion[2]) < 1e-6 ){
        if(verbose){
