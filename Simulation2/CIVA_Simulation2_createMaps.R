# Thu Nov  9 10:17:25 2023 ------------------------------
# JD
# What: CIVA simulation 2. Realistic maps and dependency over sessions


library(CICA)
n <- readNifti('C:/Users/jeffr/OneDrive - Erasmus University Rotterdam/Data/23x_all_beckmann_8_and_csf_and_wm.nii.gz')
mask <- readNifti('C:/Users/jeffr/OneDrive - Erasmus University Rotterdam/Data/brainmask_23_28_23.nii.gz')
mask <- matrix(mask)
maskid <- which(mask == 1)

dim(n)
Nif <- matrix(n, ncol = dim(n)[4])
#view(n[,,,6])
#view(n[,,,5])
#view(mask)



for(i in 1:10){
  set.seed(i)
  S1 <- Nif[maskid,c(1,5)] #### med vis/ DMN they deteriorate
  S2 <- Nif[maskid,c(3,6)] #### stable networks over sessions
  
  
  #cluster 1 corr signals
  # session 2
  #set.seed(2407)
  
  sig1 <- which(S1[,1]!=0)
  zeroid <- sample(sig1, size = ceiling(length(sig1)*.15) ) #~ 91
  S1_1ses2 <- S1[,1]
  S1_1ses2[zeroid] <- 0 
  cor(S1[,1], S1_1ses2)
  
  sig1 <- which(S1[,2]!=0)
  zeroid <- sample(sig1, size = ceiling(length(sig1)*.15) ) #~ 91
  S1_2ses2 <- S1[,2]
  S1_2ses2[zeroid] <- 0 
  cor(S1[,2], S1_2ses2)
  
  
  
  # session 3
  #set.seed(2407)
  sig1 <- which(S1_1ses2!=0)
  zeroid <- sample(sig1, size = ceiling(length(sig1)*.4) ) #~ 91
  S1_1ses3 <- S1_1ses2
  S1_1ses3[zeroid] <- 0 
  cor(S1[,1], S1_1ses2) #.90
  cor(S1_1ses2, S1_1ses3) #.79
  cor(S1[,1], S1_1ses3) #.72
  
  #set.seed(2407)
  sig2 <- which(S1_2ses2!=0)
  zeroid <- sample(sig2, size = ceiling(length(sig2)*.4) ) #~ 91
  S1_2ses3 <- S1_2ses2
  S1_2ses3[zeroid] <- 0 
  cor(S1[,2], S1_2ses2) #.91
  cor(S1_2ses2, S1_2ses3) #.74
  cor(S1[,2], S1_2ses3) #.67
  
  
  
  #cluster 2 corr signals ### make a relatively stable cluster
  # session 2
  #set.seed(2407)
  
  sig1 <- which(S2[,1]!=0)
  zeroid <- sample(sig1, size = ceiling(length(sig1)*.05) ) #~ 91
  S2_1ses2 <- S2[,1]
  S2_1ses2[zeroid] <- 0 
  cor(S2[,1], S2_1ses2) #.96
  
  sig1 <- which(S2[,2]!=0)
  zeroid <- sample(sig1, size = ceiling(length(sig1)*.05) ) #~ 91
  S2_2ses2 <- S2[,2]
  S2_2ses2[zeroid] <- 0 
  cor(S2[,2], S2_2ses2) #.97
  
  
  
  # session 3
  #set.seed(2407)
  sig1 <- which(S2_1ses2!=0)
  zeroid <- sample(sig1, size = ceiling(length(sig1)*.05) ) #~ 91
  S2_1ses3 <- S2_1ses2
  S2_1ses3[zeroid] <- 0 
  cor(S2[,1], S2_1ses2) #.90
  cor(S2_1ses2, S2_1ses3) #.97
  cor(S2[,1], S2_1ses3) #.93
  
  #set.seed(2407)
  sig2 <- which(S2_2ses2!=0)
  zeroid <- sample(sig2, size = ceiling(length(sig2)*.05) ) #~ 91
  S2_2ses3 <- S2_2ses2
  S2_2ses3[zeroid] <- 0 
  cor(S2[,2], S2_2ses2) #.97
  cor(S2_2ses2, S2_2ses3) #.97
  cor(S2[,2], S2_2ses3) #.94
  
  
  #### combine signals in array
  library(abind)
  #S1 
  QS11 <- S1
  QS12 <- cbind(S1_1ses2, S1_2ses2)
  QS13 <- cbind(S1_1ses3, S1_2ses3)
  
  #S2 
  QS21 <- S2
  QS22 <- cbind(S2_1ses2, S2_2ses2)
  QS23 <- cbind(S2_1ses3, S2_2ses3)
  
  S1 <- abind(t(QS11),t(QS12),t(QS13), along = 3)
  S2 <- abind(t(QS21),t(QS22),t(QS23), along = 3)
  
  # cor(t(S1[,,1]), t(S1[,,2]))
  # cor(t(S1[,,2]), t(S1[,,3]))
  # cor(t(S1[,,1]), t(S1[,,3]))
  # 
  # 
  # cor(t(S2[,,1]), t(S2[,,2]))
  # cor(t(S2[,,2]), t(S2[,,3]))
  # cor(t(S2[,,1]), t(S2[,,3]))
  # 
  ext <- paste('C:/Users/jeffr/OneDrive - Erasmus University Rotterdam/Documents/CIVA/CIVA/Simulation2/Maps/',
        'Maps_S1_',i,'.Rdata', sep = '')
  save(S1, file = ext)
  
  ext <- paste('C:/Users/jeffr/OneDrive - Erasmus University Rotterdam/Documents/CIVA/CIVA/Simulation2/Maps/',
               'Maps_S2_',i,'.Rdata', sep = '')
  save(S2, file = ext)
}
